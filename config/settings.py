"""Application configuration and runtime context helpers."""
...
class Config:
    ...
    DEFAULT_FAMILY_NUMBERS: list[str] = ["917766905770"]
    DEFAULT_SHOP_ORDER: list[str] = [
        "SITARAM AND SONS",
        "SITARAM SHANKAR LAL",
    ...
    def load_client_config(cls, file_name: str = "client_config.yaml") -> dict[str, Any]:
        """Load optional client-specific config from YAML."""
        ...
        if not isinstance(data, dict):
            raise ValueError("client_config.yaml must be a mapping/dictionary.")
        return data

    @classmethod
    def family_numbers(cls) -> list[str]:
        """Return destination WhatsApp numbers from client config or defaults."""
    ...
    def shop_order(cls) -> list[str]:
        """Return fixed shop order from client config or defaults."""
        client_cfg = cls.load_client_config()
